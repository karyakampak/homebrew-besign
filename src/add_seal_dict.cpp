...
std::string get_page_seal(const std::vector<uint8_t>& pdf_content, const std::string& object_page_ref) {
...
bool containsAnnots_seal(const std::vector<uint8_t>& pdfData) {
...
std::unordered_map<std::string, std::string> checkAnnotOnPage_seal(std::string page_dict) {
    ...
    if (containsAnnots_seal(page_vec)) {
...
std::string get_catalog_seal(const std::vector<uint8_t>& pdf_content, const std::unordered_map<std::string, std::string>& read_pdf) {
    std::vector<uint8_t> object_page;
    std::vector<uint8_t> pdf = pdf_content;
    std::string raw_string_doc(pdf_content.begin(), pdf_content.end());
...
size_t extract_integer_seal(const std::string& input) {
...
std::string size_t_to_string_seal(size_t value) {
    return std::to_string(value);
}

// Convert a vector of size_t values to a string
std::string vector_to_string_seal(const std::vector<size_t>& input) {
    std::string output;
    for (size_t i = 0; i < input.size(); ++i) {
        output += size_t_to_string_seal(input[i]);
        if (i < input.size() - 1) {
...
std::unordered_map<std::string, std::vector<uint8_t> > AddSealDict::add_sealdict(std::vector<unsigned char> pdf_content, const char* input_image_path_string, int page, int visibility, float x, float y, float width, float height, std::vector<std::string> object_page_ref, std::unordered_map<std::string, std::string> read_pdf, std::unordered_map<std::string, std::string> check_sign) {
    ...
    // std::string signature_dict = object_signature_start + "/Type /Sig\n/Filter /Adobe.PPKLite\n/SubFilter /adbe.pkcs7.detached\n/ByteRange [0 /********** /********** /**********]\n" + contents + "\n/Reason (Signed Certificate.)\n/M (D:" + formatted_timestamp_with_z + ")\n/ContactInfo (sign@example.com)\n/Name (Example)\n/Location (Jakarta)\n/Reference [<</TransformMethod/DocMDP/TransformParams <</P 3/Type/TransformParams/V/1.2>>/Type/SigRef>> <</TransformMethod/FieldMDP/TransformParams <</Action/Include/Fields[]/P 3/Type/TransformParams/V/1.2>>/Type/SigRef>>]" + object_signature_end;
    std::string signature_dict = object_signature_start + "/Type /Sig\n/Filter /Adobe.PPKLite\n/SubFilter /ETSI.CAdES.detached\n/ByteRange [0 /********** /********** /**********]\n" + contents + "\n/Reason (Signed Certificate.)\n/M (D:" + formatted_timestamp_with_z + ")\n/ContactInfo (sign@example.com)\n/Name (Example)\n/Location (Jakarta)\n/Reference [<</TransformMethod/DocMDP/TransformParams <</P 3/V/1.2>>/Type/SigRef>>]" + object_signature_end;
    ...
    std::string page_dict = get_page_seal(pdf_content_page_dict, object_page_ref_annot);
    std::unordered_map<std::string, std::string> annots_on_page = checkAnnotOnPage_seal(page_dict);
    ...
    std::string catalog_dict = get_catalog_seal(pdf_content_catalog_dict, read_pdf_dict);
    ...
    size_t rootIndex = extract_integer_seal(read_pdf.at("rootRef"));
    size_t pageIndex = extract_integer_seal(page_dict.substr(0, 10));
    ...
    std::string added_index_string = vector_to_string_seal(x_list);
